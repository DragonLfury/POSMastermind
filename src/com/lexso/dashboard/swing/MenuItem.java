package com.lexso.dashboard.swing;
...
public class MenuItem extends javax.swing.JPanel {
    ...
    private void initSubMenu(ModelMenu menu, String currentRole) {
        subMenu = new javax.swing.JPanel();
        subMenu.setLayout(new MigLayout("wrap, insets 0, gapy 0", "fill"));

        for (int i = 0; i < menu.getSubMenu().length; i++) {
            String subMenuName = menu.getSubMenu()[i];
            SubMenuItem item = new SubMenuItem(subMenuName);

            boolean enabled = menu.isSubMenuAllowed(subMenuName, currentRole);
            item.setEnabled(enabled);

            if (!enabled) {
                item.setForeground(Color.GRAY);
                item.setToolTipText("Requires higher privileges");
            }

            item.addActionListener(e -> {
                if (enabled && event != null) {
                    event.menuSelected(index, i);
                }
            });

            subMenu.add(item, "h 35!");
        }
    }
