 ...
 */
package com.lexso.settings.location.district;
...
public class DistrictManagement extends javax.swing.JPanel {
    ...
    public void loadDistrict() {
        loadDistrict(""); // Call the overloaded method with empty search term
    }

    public void loadDistrict(String searchTerm) {
        try {
            String query;
            if (searchTerm.isEmpty()) {
                query = "SELECT * FROM `district` INNER JOIN `status` ON `district`.`status_Id` = `status`.`id` INNER JOIN `province` ON `district`.`province_id` = `province`.`id` INNER JOIN `country` ON `province`.`country_id` = `country`.`id` ORDER BY `district`.`id` ASC ";
            } else {
                query = "SELECT * FROM `district` INNER JOIN `status` ON `district`.`status_Id` = `status`.`id` INNER JOIN `province` ON `district`.`province_id` = `province`.`id` INNER JOIN `country` ON `province`.`country_id` = `country`.`id` "
                        + "WHERE `district_name` LIKE '%" + searchTerm + "%' ORDER BY `district`.`id` ASC ";
            }
    ...
    private void initComponents() {
        ...
        jLabel2.setText("Search District");
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "id", "District", "Province", "Country", "Status"
            }
        ) {
...
//        jComboBox2.setModel(new DefaultComboBoxModel(new Vector())); // Clear the combo box
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        try {
            ...
            ResultSet resultset = DatabaseConnection.executeSearch("SELECT * FROM `district` WHERE `district_name` = '" + district + "' AND  `province_id` = '" + provinceMap.get(province) + "'");
            ...
            if (country == "Select Country") {
                ...
                JOptionPane.showMessageDialog(this, "This District Already added", "Warning", JOptionPane.WARNING_MESSAGE);
            } else {
                DatabaseConnection.executeIUD("INSERT INTO `district` (`district_name`,`province_id`,`status_Id`)VALUES('" + district + "','" + provinceMap.get(province) + "','1')");
                reset();
                loadDistrict();
            }
        } catch (Exception e) {
    ...
    private void jTable1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTable1MouseClicked
        try {
            ...
            String district = String.valueOf(jTable1.getValueAt(row, 1));
            String province = String.valueOf(jTable1.getValueAt(row, 2));
            String country = String.valueOf(jTable1.getValueAt(row, 3));
            jComboBox1.setSelectedItem(country);
            jComboBox2.setSelectedItem(province);
            jTextField1.setText(district);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        try {
            ...
            if (country == "Select Country") {
                ...
                DatabaseConnection.executeIUD("Update `district` SET `district_name` = '" + district + "',`province_id`= '" + provinceMap.get(province) + "'  WHERE `id` = '" + id + "'");
                reset();
                loadDistrict();
            }
        } catch (Exception e) {
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
         try {
            int row = jTable1.getSelectedRow();
            String id = String.valueOf(jTable1.getValueAt(row, 0));
            ResultSet resultset1 = DatabaseConnection.executeSearch("SELECT * FROM `district` WHERE `id` = '" + id + "' ");

            if (resultset1.next()) {
                int status = resultset1.getInt("status_Id");

                if (status == 1) {
                    DatabaseConnection.executeIUD("UPDATE `district` SET `status_Id` = '2' WHERE `id` = '" + id + "'");

                } else {
                    DatabaseConnection.executeIUD("UPDATE `district` SET `status_Id` = '1' WHERE `id` = '" + id + "'");
